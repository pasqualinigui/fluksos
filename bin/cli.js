...
function printStackHelp(stackName) {
  ...
  console.log(`\x1b[1m\x1b[32m=== ${stackName.toUpperCase()} STACK ===\x1b[0m\n`)
...
switch (command) {
  ...
  case 'init': {
    ...
    if (!stack || !STACK_REGISTRY[stack]) {
      console.error('\x1b[31m[ERROR] You must specify a valid stack to initialize.\x1b[0m')
      console.log(`Available stacks: \x1b[32m${Object.keys(STACK_REGISTRY).join(', ')}\x1b[0m`)
      console.log('Example: fluksos init \x1b[32mnextjs\x1b[0m my-app --tier 3')
      process.exit(1)
    }
  ...
  case 'generate': {
    ...
    if (!stack || !STACK_REGISTRY[stack]) {
      console.error('\x1b[31m[ERROR] You must specify a valid stack for generators.\x1b[0m')
      console.log(`Available stacks: \x1b[32m${Object.keys(STACK_REGISTRY).join(', ')}\x1b[0m`)
      console.log('Example: fluksos generate \x1b[32mnextjs\x1b[0m action CreateUser ./src/actions')
      process.exit(1)
    }
  ...
  case 'validate': {
    ...
    if (!stackArg || (stackArg !== 'all' && !STACK_REGISTRY[stackArg])) {
      console.error(`\x1b[31m[ERROR] You must specify a valid stack (or "all") to validate.\x1b[0m`)
      console.log(
        `Available options: \x1b[32mall, ${Object.keys(STACK_REGISTRY).join(', ')}\x1b[0m`,
      )
      console.log('Example: fluksos validate \x1b[32mnextjs\x1b[0m ./my-project')
      process.exit(1)
    }
