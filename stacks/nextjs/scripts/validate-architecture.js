...
function walk(dir) {
  try {
    ...
    return entries.flatMap((entry) => {
      ...
      if (stat.isDirectory()) {
        if (['node_modules', '.git', '.next', '.turbo', '.agent', '.agents', 'dist', 'build', 'out'].includes(entry)) return [];
...
function findAppRoots(dir, roots = []) {
  try {
    const entries = readdirSync(dir);
    if (entries.includes('next.config.ts') || entries.includes('next.config.js') || entries.includes('next.config.mjs')) {
      roots.push(dir);
    }
    for (const entry of entries) {
      if (['node_modules', '.git', '.next', '.turbo', '.agent', '.agents', 'dist', 'build', 'out'].includes(entry)) continue;
      const fullPath = join(dir, entry);
      const stat = statSync(fullPath);
      if (stat.isDirectory()) {
...
export function validateArchitecture(rootDir) {
  ...
  for (const appRoot of appRoots) {
    ...
    if (existsSync(join(appRoot, 'src', 'middleware.ts')) || existsSync(join(appRoot, 'middleware.ts'))) {
      violations.push({
        file: join(relRoot, 'middleware.ts').replace(/\\/g, '/'),
        rule: 'deprecated-middleware',
        message: 'FATAL: Next.js 16.2+ no longer uses middleware.ts. You MUST use proxy.ts for edge routing.',
        severity: 'FATAL'
      });
    }
    ...
    if (!existsSync(join(appRoot, 'src', 'app', 'llms.txt')) && !existsSync(join(appRoot, 'src', 'app', 'llms.txt', 'route.ts'))) {
      violations.push({
    ...
    if (!existsSync(join(appRoot, 'src', 'app', 'llms-full.txt')) && !existsSync(join(appRoot, 'src', 'app', 'llms-full.txt', 'route.ts'))) {
      violations.push({
    ...
    if (existsSync(join(appRoot, 'public', 'robots.txt'))) {
      violations.push({
        ...
        message: 'Static robots.txt is forbidden. Use dynamic src/app/robots.ts for environment awareness.',
        severity: 'FATAL'
      });
    }
    ...
    if (existsSync(join(appRoot, 'tailwind.config.ts')) || existsSync(join(appRoot, 'tailwind.config.js'))) {
      violations.push({
    ...
    if (existsSync(join(appRoot, 'postcss.config.js')) || existsSync(join(appRoot, 'postcss.config.cjs'))) {
      violations.push({
  ...
  for (const file of files) {
    ...
    if (relPath.includes('(marketing)') && relPath.endsWith('page.tsx')) {
      const dir = join(file, '..');
      if (!existsSync(join(dir, 'opengraph-image.png')) && 
          !existsSync(join(dir, 'opengraph-image.jpg')) && 
          !existsSync(join(dir, 'opengraph-image.tsx'))) {
        violations.push({
          file: relPath,
          rule: 'missing-opengraph-image',
          message: 'Public indexable pages MUST have an opengraph-image.(png|jpg|tsx) in the same directory.',
          severity: 'ERROR'
        });
      }
    }
