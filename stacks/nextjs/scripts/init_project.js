...
async function applyObservabilityTemplates(ctx) {
  ...
  if (fs.existsSync(path.join(ctx.templatesRoot, 'tests'))) {
    copyDirTemplate(path.join(ctx.templatesRoot, 'tests'), path.join(ctx.workspaceRoot, 'tests'));
  }
}
...
async function setupLefthook(ctx) {
  ...
  try {
    runCommand('pnpm', ['exec', 'lefthook', 'install'], { cwd: ctx.workspaceRoot });
  } catch {
    ctx.log.warn('Could not run lefthook install. Run manually: pnpm exec lefthook install');
  }
}
...
function assertRequiredTemplateFiles(ctx) {
  ...
  const requiredFiles = [
    ...
    path.join(ctx.templatesRoot, 'root', 'docker-compose.yml'),
  ...
  if (ctx.tier >= 2) {
    requiredFiles.push(path.join(ctx.templatesRoot, 'app-tier-2', 'src', 'lib', 'safe-action.ts'));
    requiredFiles.push(
      path.join(ctx.templatesRoot, 'app-tier-2', 'src', 'lib', 'fetcher.server.ts')
    );
  }

  if (ctx.tier === 3) {
    requiredFiles.push(path.join(ctx.templatesRoot, 'app-tier-3', 'drizzle.config.ts'));
    requiredFiles.push(path.join(ctx.templatesRoot, 'app-tier-3', 'src', 'db', 'index.ts'));
    requiredFiles.push(path.join(ctx.templatesRoot, 'app-tier-3', 'src', 'lib', 'auth.ts'));
    requiredFiles.push(
      path.join(
        ctx.templatesRoot,
        'app-tier-3',
...
async function validateScaffold(ctx) {
  const invariants = [
    ...
    path.join(ctx.workspaceRoot, 'pnpm-workspace.yaml'),
    path.join(ctx.workspaceRoot, 'docker-compose.yml'),
...
async function main() {
  ...
  ctx.log.info(`Scaffolding senior Next.js workspace: ${ctx.projectName} (Tier ${ctx.tier})`);

  await runStep(ctx, 'Assert preconditions', () => assertPreconditions(ctx));
  await runStep(ctx, 'Assert required template files', () => assertRequiredTemplateFiles(ctx));
  await runStep(ctx, 'Create workspace root', () => createWorkspaceRoot(ctx));
  await runStep(ctx, 'Scaffold Next app', () => scaffoldNextApp(ctx));
  await runStep(ctx, 'Cleanup generated files', () => cleanupGeneratedFiles(ctx));
  await runStep(ctx, 'Install common dependencies', () => installCommonDependencies(ctx));
  await runStep(ctx, 'Install tier dependencies', () => installTierDependencies(ctx));
  await runStep(ctx, 'Apply root templates', () => applyRootTemplates(ctx));
  await runStep(ctx, 'Apply common app templates', () => applyCommonAppTemplates(ctx));
  await runStep(ctx, 'Apply tier templates', () => applyTierTemplates(ctx));
  await runStep(ctx, 'Apply observability templates', () => applyObservabilityTemplates(ctx));
